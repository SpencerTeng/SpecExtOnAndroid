...
//		}
	...
	// Double
	// ------
	public int readFramesdd(double[] sampleBuffer, int numFramesToRead, int numFramesToJump) throws IOException, WavFileException
	{
		return readFrames(sampleBuffer, 0, numFramesToRead);
	}
	...
	public int readFramesDownSampled(double[] sampleBuffer, int offset, int numFramesToRead, int factor) throws IOException, WavFileException
	{
		if (ioState != IOState.READING) throw new IOException("Cannot read from WavFile instance");

		for (int f=0 ; f<numFramesToRead ; f+=factor)
		{
			if (frameCounter == numFrames) return f;
