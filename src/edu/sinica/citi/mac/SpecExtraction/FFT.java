 ...
 * the application of a Hamming window.
 *
...
public class FFT
{
	...
	//	int valid_size = jAudioFeatureExtractor.GeneralTools.Statistics.ensureIsPowerOfN(real_input.length, 2);
		...
		int valid_size = real_input.length;
		if (valid_size % 2 != 0)
				throw new Exception("number of samples need to be a power of 2...");
		//System.out.println("real_input length: " + real_input.length);
		...
		// Apply a Hamming window to the real values if this option is
		// selected
		if (use_hanning_window)
		{
			for (int i = 0; i < real_output.length; i++)
			{
				double hamming = 0.54 - 0.46 * Math.cos(2 * Math.PI * i / valid_size);
				real_output[i] *= hamming;
			}
		}
	...
	public double[] getMagnitudeSpectrum()
	{
		...
		{
			int number_unfolded_bins = imaginary_output.length / 2 + 1;
			output_magnitude = new double[number_unfolded_bins];
			for(int i = 0; i < output_magnitude.length; i++)
				output_magnitude[i] = ( Math.sqrt(real_output[i] * real_output[i] + imaginary_output[i] * imaginary_output[i]) ) ;
		}
		
		
		//db scale
		//for(int i=0; i < output_magnitude.length; i++)
		//	output_magnitude[i] = Math.log10(output_magnitude[i]+0.00001);
	...
	public double[] getBinLabels(double sampling_rate)
	{
		...
		double bin_width = sampling_rate / number_bins;
		int number_unfolded_bins = imaginary_output.length / 2;
